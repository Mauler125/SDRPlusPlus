...
#define COLOR_WHITE "\x1B[0m"
    ...
    void __log__(Type type, const char* fmt, const std::vector<std::string>& args) {
        ...
        // Write to output
        {
            ...
            fprintf(outStream, "[%02d/%02d/%02d %02d:%02d:%02d.%03d] [", nowc.tm_mday, nowc.tm_mon + 1, nowc.tm_year + 1900, nowc.tm_hour, nowc.tm_min, nowc.tm_sec, (int)ms);
            ...
            __android_log_print(TYPE_PRIORITIES[type], FLOG_ANDROID_TAG, COLOR_WHITE "[%02d/%02d/%02d %02d:%02d:%02d.%03d] [%s%s" COLOR_WHITE "] %s\n",
                    nowc.tm_mday, nowc.tm_mon + 1, nowc.tm_year + 1900, nowc.tm_hour, nowc.tm_min, nowc.tm_sec, (int)ms, TYPE_COLORS[type], TYPE_STR[type], out.c_str());
#else
            // Print format string
